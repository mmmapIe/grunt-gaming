...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'finale': //gamer 2
				var bg:BGSprite = new BGSprite('gaming stage/week2bg', -600, -200, 0.9, 0.9);
				add(bg);

			case 'gamingnormal': //gamer
				var bg:BGSprite = new BGSprite('gaming stage/normalback', -600, -200, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('gaming stage/normalfront', -650, 600, 0.9, 0.9);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
				stageFront.updateHitbox();
				add(stageFront);

				case 'gamingangry': //gamer
				var bg:BGSprite = new BGSprite('gaming stage/angryback', -600, -200, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('gaming stage/angryfront', -650, 600, 0.9, 0.9);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
				stageFront.updateHitbox();
				add(stageFront);

				case 'gaminggod': //gamer
				var bg:BGSprite = new BGSprite('gaming stage/godback', -600, -200, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('gaming stage/godfront', -650, 600, 0.9, 0.9);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
				stageFront.updateHitbox();
				add(stageFront);
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				case "monster":
					...
					case "rage-quit": 
					startVideo('rage quit');

					case "exploits": 
					startVideo('exploits');

					case "break-in": 
					startVideo('break in720');
					...
					if(daNote.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
						...
						switch (Math.abs(daNote.noteData))
                        {
                            case 2:
                                ...
                                if(SONG.player2 == 'gruntgamingisgod')
                                    {
                                        health -= 0.006;
                                    }
                                if(SONG.player2 == 'finalgaming')
                                    {
